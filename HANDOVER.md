...
- `index.html` + `js/twin.js` + `js/scene.js` + `js/sim.js` — Three.js 3D twin, no build step, WebSocket. The side panel is three tabs (Now · Try it · Tide & rig; `?tab=try` opens one), a first-visit guide explains the page and every status badge (`?welcome=0` suppresses it, as do `?sim=` and `?demo=`), and every chip explains itself on hover. `sim.js` is the simulation drawer: the counterfactual on a real axis, scrubbable hour by hour, driving the 3D water; `?sim=1&scenario=…&trace=opt&hour=N&storm=1.5` opens it on load and `&demo=1` loops it. With no API it reads the bundle `scripts/bake_dashboard_data.py` writes to `assets/sim_*.json` and says BUNDLED. To look at any of this without a person at the screen, `scripts/drive_dashboard.py URL out.png [--wait-sim] [JS…]` drives Chrome over the DevTools protocol and reads the DOM back
