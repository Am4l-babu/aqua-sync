...
| Test suite & CI | ✅ Done | Am4l-babu | `feature/api-and-validation` | **97** physics/behaviour tests passing (10 Sep). **`python scripts/check.py` is the single gate** — lint, tests, a glyph audit for characters the PDF fonts drop silently, plus regeneration and determinism of every figure and document. Runs automatically via `.githooks/pre-commit` and the CI `selfcheck` job |
