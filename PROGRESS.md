...
| Test suite & CI | ✅ Done | Am4l-babu | `feature/api-and-validation` | **139** physics/behaviour tests passing (24 Sep). **`python scripts/check.py` is the single gate** — lint, tests, a glyph audit for characters the PDF fonts drop silently, plus regeneration and determinism of every figure and document. Runs automatically via `.githooks/pre-commit` and the CI `selfcheck` job |
...
| Level sensing + EKF | 📋 Todo | — | — | JSN-SR04T + DS18B20 temperature compensation. **No longer blocked — danger fixed 9 Sep, business decision still open separately.** `main.cpp`'s `readPressureDepth()` expects a 0–5 V hydrostatic pressure transducer — a V3 BOM item (`hardware/bom/bom.html`, ₹1,800) not in V1 — and used to seed the boot-time level estimate from it and trust it *more* than the ultrasonic reading on disagreement. Since the floating ADC pin could pass its own sanity check on RF/PWM noise alone, that meant `sensors_agree` could flip with no fault switch touched. **`GPIO36` is now safe to leave unconnected**: `HAS_PRESSURE_SENSOR` in `config.h` is undefined by default, `readPressureDepth()` returns `NAN` instead of reading the floating pin, and the boot seed and `sensors_agree` both fall back to the one real sensor honestly. Compiles clean with the flag on or off. What is still open is the **business call**: order the ₹1,800 part for genuine two-physics fusion, or keep it ultrasonic-only for the demo. [PR #28](https://github.com/Am4l-babu/aqua-sync/pull/28) |
...
| Component | Status | Assigned To | Branch | Notes |
|---|---|---|---|---|
| 3D twin dashboard | ✅ Done | Am4l-babu | `feature/real-terrain-3d-twin` | Rebuilt 8 Sep on **measured ground**: `scripts/build_terrain.py` bakes 18 km of the real Periyar valley from the DEM tiles the catchment work already caches (448², 40 m posting), and the reservoir footprint is derived from the DEM's flat water sheet rather than drawn, so the shoreline walks up actual topography as the level moves. Shader water, sky, fog, and a procedural arch dam sized off the real 365.85 m crest. A caption states which parts are measured and which are schematic. Verified in headless Chrome. [PR #13](https://github.com/Am4l-babu/aqua-sync/pull/13). **Defect found and fixed 9 Sep:** the badge lit green **LIVE** the moment the WebSocket opened, while the stream underneath was a recorded October 2021 episode — the socket being open says the backend is reachable, not that anything is measured. Provenance is now declared by the server on every frame (`source`, defaulting to `REPLAY` so a forgetful path under-claims) and the dashboard displays what it is told; only LIVE is green. Six tests pin it. [PR #24](https://github.com/Am4l-babu/aqua-sync/pull/24). **Offline demo verified 9 Sep** — served with no backend at all, the scene renders, the bundled replay drives it and the recommendation still comes out, which is demo beat 6. That exposed `pollRig` logging a 404 per second when offline; it now backs off to 15 s and recovers without a reload. [PR #25](https://github.com/Am4l-babu/aqua-sync/pull/25). **9 Sep, ground imagery:** the procedural green-to-rock shading is replaced by a real Sentinel-2 photograph (43PGL, 7 Feb 2024, 1.28% cloud), warped onto the terrain's own window so it lands pixel-for-pixel on the heightmap. It is never drawn over the reservoir — the mesh already drops every triangle inside the water, so the frozen photographed shoreline cannot disagree with the level the twin animates. Caption claims "Ground: measured" only when the texture actually loaded; verified by removing the asset and watching the claim disappear. [PR #32](https://github.com/Am4l-babu/aqua-sync/pull/32). **11 Sep, drawn as photographed:** the photograph was being lit a second time by the scene's sun, sky fill, shadow map and filmic tone curve, which turned the forest a green the picture does not contain. With imagery present the ground is now unlit and untone-mapped, with a gentle DEM hillshade for relief, and the water is toned to the reservoir's real dark teal. Site / Gate / Basin buttons over the view. Committed on `fix/dashboard-legibility`, browser-verified over the DevTools protocol. **24 Sep, readable to a first-time visitor** (`fix/dashboard-legibility`, `ca0b3a2`, not yet merged): a first-visit guide with a glossary and badge meanings; the panel split into three tabs (Now, Try it, Tide & rig) so the interactive cards are no longer below the fold; hover definitions for FRL, m MSL and cumecs, and every status chip explains itself; "cushion below FRL" throughout, the server's advice sentence included; the provenance caption cut to one line carrying all three claims, the paragraph a click away; time to FRL in words, not "2158.7 d"; what-if presets; tablets and phones now scroll instead of squeezing the panel to a sliver. Driven over the DevTools protocol at four screen sizes, online and offline, no page errors. The rig tab's fault dot is **not** browser-verified - no fault source on this machine |
| FastAPI backend | ✅ Done | Am4l-babu | `feature/api-and-validation` | Eleven REST routes (health, reservoirs, scenarios, counterfactual — now with `?inflow_scale=` for the storm stress test — what-if, crisis briefing, crisis score, tide, and from 11 Sep `runoff` for the curve-number what-if and `stress_sweep` serving `scripts/stress_sweep.py`'s file) + telemetry WebSocket fan-out; every frame and what-if answer also carries `advice_ml`. Serves the dashboard same-origin. **9 Sep:** the counterfactual route was built and tested but never called from the dashboard — same for tide. Both are now driven from the panel (next row and the one after). The counterfactual runs an exhaustive policy search taking 11–17 s and depended on nothing in the request, so it is now cached (`lru_cache`, the same pattern `twin/crisis.py` already used): 12.1 s cold, 0.0017 s warm. [PR #31](https://github.com/Am4l-babu/aqua-sync/pull/31) |
| What-if panel | ✅ Done | Am4l-babu | `feature/whatif-wiring` | Wired 8 Sep — the slider POSTs to `/api/whatif` on release and the card shows the 72 h peak level, cushion at peak, whether it reaches FRL, and the advice sentence. Contract checked with curl against a running API (higher release → lower peak, larger cushion). **Browser-verified 8 Sep** — the card renders and hides correctly. [PR #10](https://github.com/Am4l-babu/aqua-sync/pull/10), merged into `development`. **Defect found and fixed 9 Sep:** the card showed "Cushion at peak" one line above an advice sentence computed at the *end* of the 72 h horizon — under a large release those are different moments, and the card read as internally contradictory (one case: "4.38 m" beside "6.70 m of flood cushion" with nothing to say why). Both moments are now labelled: a "Level at 72 h" row joins the peak, and the advice line is prefixed "At 72 h:". [PR #33](https://github.com/Am4l-babu/aqua-sync/pull/33), merged into `development` — **not yet promoted to `main`** at time of writing |
| Twin simulation panel | ✅ Done | Am4l-babu | `feature/dashboard-tide-and-simulation` | 9 Sep. Replays the flagship episode against the optimiser's policy and draws both level traces against FRL, with the headline stated as cushion — **about 3 m**, in whole metres because replay MAE is 0.30 m — never as peak reduction, which is −277% on this episode and means nothing, because neither schedule reached bankfull; the API's own `headline_note` is reproduced verbatim rather than paraphrased. Revenue row labelled "vs the day" to match Crisis Commander's wording for the same underlying number, since it is a different comparison from `lead_time_headline.json`'s 4.4–9.6 cr range. [PR #31](https://github.com/Am4l-babu/aqua-sync/pull/31). **11 Sep, simulation drawer** (`dashboard/js/sim.js`, on `fix/dashboard-legibility`): a Simulation button over the 3D view (or `?sim=1&trace=opt&hour=N`) opens the episode on real axes — recorded bulletin level, the twin's replay and the optimiser's schedule, with inflow and both releases — and a scrubber that drives the 3D water and the readouts hour by hour under a SIMULATED badge. Carries the downstream peaks the API already computed (one dam, uncalibrated reach, never against bankfull), a storm stress test at ×1.25/1.5/2 of the recorded inflow through the same optimiser (replay error and recorded line dropped for a scaled run, because they no longer apply), and the sweep from `scripts/stress_sweep.py`: the day's schedule reaches FRL at **×1.25**, AquaSync's not by ×3, because it releases up to 1,500 cumecs. A **Catchment policy** card runs the episode's rain through the SCS-CN chain at a chosen curve number against handbook 72 — runoff volume only, the peak greyed and labelled not validated (NSE 0.07 on shape). All driven in a browser over the DevTools protocol; 139 tests |
| Tide panel | ✅ Done | Am4l-babu | `feature/dashboard-tide-and-simulation` | 9 Sep. `/api/tide`'s 72 h harmonic curve with its low-water windows shaded, metres about mean sea level at Kochi — chipped `PREDICTED`, never mixed into the reservoir's m MSL readout, the same discipline the rig's tank depth already gets. [PR #31](https://github.com/Am4l-babu/aqua-sync/pull/31) |
...
| Malayalam alerting | ✅ Done | Am4l-babu | `fix/dashboard-legibility` | 11 Sep, as template wording: `_advice_ml` in `api/main.py` puts a Malayalam line on every telemetry frame and what-if answer, on the same KSDMA bands as the English advice and pinned to them by six tests; the dashboard shows it under the recommendation, and the offline replay mirrors it client-side. **Draft text written by an assistant — a native speaker must read the five sentences before any public use.** Delivery over a real channel (SMS, LDMC, Kudumbashree) stays in [ROADMAP.md](ROADMAP.md) beyond the expo |
